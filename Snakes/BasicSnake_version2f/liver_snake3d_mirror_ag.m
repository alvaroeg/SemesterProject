...
flip_feat = fliplr(feat);
eval_im = [feat flip_feat];
% imshow(eval_im(:,:,15),[])
...
if coor == 1
    ...
    FV.vertices(:,1) = FV.vertices(:,1) + floor(size(eval_im,2)/2);
    FV.vertices(:,2) = FV.vertices(:,2) + 1.1*floor(size(eval_im,1)/2);
    FV.vertices(:,3) = FV.vertices(:,3) + 1.3*floor(size(eval_im,3)/2);
    
    FV.vertices(FV.vertices(:,1)>size(eval_im,2),1) = size(eval_im,2);
    FV.vertices(FV.vertices(:,2)>size(eval_im,1),2) = size(eval_im,1);
    FV.vertices(FV.vertices(:,3)>size(eval_im,3),3) = size(eval_im,3);
...
OV=Snake3D_ag(I,FV,Options);OV=Snake3D_ag(I,FV,Options);
figure(11)
% pause
