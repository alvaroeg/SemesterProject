function dist = distance_growbox_func(level,i)
% Calculate the intensity weighted distance to sliding boundary location
% 
% SYNOPSIS: dist = distance_growbox_func(level,i)
% 
% DESCRIPTION: the gray scale distance transform of the maximum shear
% stretch image is used to calculate the distance from each manually
% selected sliding boundary point to the calculated optimum sliding
% boundary location
% 
% INPUT level: threshold used for segmentation
%       i: dataset number
% 
% OUTPUT dist: cell containing the distances for each landmark
