...
clear all
close all
load /scratch/alvaroeg/validation/feat_mm_norm.mat
...
dist = cell(size(train_set,1),11);
for train_loop = 1:size(train_set,1)
    train_loop
    lcount = 0;
    for level = 0:0.01:0.2
        level
        lcount = lcount + 1;
        dist{train_loop,lcount} = cell(size(feats_mm_norm));
        for i_count = 1:3
            i = train_set(train_loop,i_count);
            for j = 1:2
                clear P;
%                 load(['../sliding_landmarks/P_mm_' num2str(i) '_1']);
                load(['/scratch/alvaroeg/validation/sliding_landmarks/P_mm_' num2str(i) '_1']);
...
%                 feat = imgaussian(feat,3); % Image smoothing
                ...
                for pp = 1:n_landmarks
                    ...
                    dist{train_loop,lcount}{i,j}(pp) = norm(Pend_new-Plm);
                    %             keyboard
        ...
        clearvars -except dist feats_mm_norm lcount level train_set train_loop
    end
end

disp('Distances calculated')
save('/scratch/alvaroeg/validation/train/dist_threshold.mat','dist');
disp('Save completed')