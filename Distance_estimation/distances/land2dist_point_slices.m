...
count = 1;
count2 = 1;
% visit_sl = [38,41,56,57,18,15,22];
visit_sl = [15,22];
for ck = 1:length(visit_sl)
    k = visit_sl(ck);
    zSlice = k;
    ...
    title(str_title)
    hold on
    contour(feat_mask(:,:,zSlice),1,'Color','y')
    if  ~isempty(fzP)
    ...
    if  ~isempty(fzP2)
        ...
        if k < zvalues2(end)
            ...
            saveas(h,['/home/alvaroeg/SemesterProject/Figures/dataset18/Landmarks_location_fix_slices/land2points_fix_slices' num2str(k) '.jpg']);
%     pause
    close
end